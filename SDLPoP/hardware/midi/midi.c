...
#include "../../pop/common.h"//"common.h"
#include "../../pop/opl3.h" // "opl3.h"
#include "math.h"
	...
	while (frames_needed > 0) {
		if (ticks_to_next_pause > 0) {
			...
			if (num_finished_tracks >= num_midi_tracks) {
				...
				//				printf("midi_callbackk(): sound ended\n");
				SDL_LockAudio();
				midi_playing = 0;
				free_parsed_midi(&parsed_midi);
				SDL_UnlockAudio();
...
byte init_MPU(char *filename, byte use_direct_MIDI) //Initialise function!
{
	byte result;

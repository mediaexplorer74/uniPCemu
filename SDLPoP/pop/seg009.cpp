		...
		([](StorageFolder^ picturesFolder)
			{
				...
				return picturesFolder->GetFileAsync("README.md");
			}
		)*/
		.then
		([](task<StorageFolder^> sf)//([](task<StorageFile^> task)
			{
				//filename_transfer(sf::Know);
				String^ imagesFolderPath = KnownFolders::PicturesLibrary->Path;
...
bool this_file_exists(const char* filename)
{
...
#ifdef USE_REPLAY
		if (rrecording) save_recorded_replay_dialog();
#endif
#ifdef USE_MENU
...
void audio_callback(void* userdata, Uint8* stream_orig, int len_orig) 
{
	...
	if (midi_playing) 
	{
		midi_callbackk(userdata, stream, len);
	}
	else if (ogg_playing) 
	{
